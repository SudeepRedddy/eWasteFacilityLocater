...
const Centers = () => {
  ...
  const [topFiveCenters, setTopFiveCenters] = useState<(Center & { distance: number; travelTime: number })[]>([]);
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Calculate distances and travel times for the centers
              ...
              {topFiveCenters.map((center) => (
                <div key={center.id} className="bg-black p-4 rounded-lg shadow-md hover:shadow-lg transition border border-gray-700">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <Button size="sm" className="flex-1 bg-[#3c9c1c] text-white hover:bg-[#3c9c1c]/90">
                      Schedule Pickup
                    </Button>
                  </div>
                </div>
